...
export const registerController = async (req, res) => {
    try {
        const { name, email, password, phone, address1, address2, pincode, area, lastName } = req.body
        // validation
        if (!name) {
        ...
        if (!area) {
            return res.send({ message: "area is required" })
        }
        if (!email) {
        ...
        if (!address1) {
            return res.send({ message: "address1 is required" })
        }
        if (!address2) {
            return res.send({ message: "address2 is required" })
        }
        if (!pincode) {
