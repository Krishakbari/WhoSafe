  ...
  return (
    <section className="py-12 px-4 max-w-7xl mx-auto font-[Nunito Sans]">
      ...
      <p className="text-sm text-center text-gray-500 mb-10">
        Stay informed with the latest headlines, real stories, and timely
        updates. Stay updated. Stay empowered.
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {blogs.map((blog) => (
          <div
            key={blog._id}
            className="bg-[#FFE8F0] rounded-xl overflow-hidden shadow-md hover:scale-[1.01] transition-transform"
          >
            <img
              src={`${API}${blog.image}`}
              alt={blog.title}
                      ...
                      if (
                        ...
                        try {
                          ...
                          const res = await axios.delete(
                            `${import.meta.env.VITE_API}/blog/${blog._id}`,
                            {
                              headers: {
